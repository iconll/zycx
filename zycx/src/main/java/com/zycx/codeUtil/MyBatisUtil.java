...
public class MyBatisUtil {
	...
	public static String genMapperConfig(TableModel table,String namespace, String beanName, String queryModelName){
		...
		//生成根据实体类属性是否有值而修改对应的属性
		sb.append(genUpdateByPrimaryKeySelective(beanName, table));
	 ...
	 * @return
	 */
	private static String genUpdateByPrimaryKeySelective(String beanName, TableModel table){
		List<ColumnModel> columnModelList = table.getColumns();
		List<ColumnModel> primaryKeys = table.getPrimaryKeyColumns();
		StringBuffer sb = new StringBuffer();
		sb.append("\t<!--updateByPrimaryKeySelective-->\n");
		sb.append("\t<update id=\"updateByPrimaryKeySelective\" parameterType=\""+beanName+"\">\n");
		sb.append("\t\tUPDATE "+table.getTableName()+" \n\t\t<set> \n");
		for(ColumnModel cm : columnModelList){
			if(!cm.isPrimaryKey()){
				sb.append("\t\t\t<if test=\""+cm.getColumnName()+" != null and "+cm.getColumnName()+" != ''\">\n");
