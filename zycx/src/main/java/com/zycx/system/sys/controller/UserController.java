...
import com.zycx.system.sys.service.UserAssociateRoleService;
import com.zycx.system.sys.service.UserRoleService;
import com.zycx.system.sys.service.UserService;
...
public class UserController extends GenericController<User,QueryUser> {
    ...
    @RequestMapping(value = "findCurrentLoginUserTreeCodeList",method = RequestMethod.GET,produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public List<String> findCurrentLoginUserTreeCodeList(Authentication au){
        User user = (User)au.getPrincipal();

        return userService.findCurrentLoginUserTreeCodeList(user);
    }
    /**
     * 修改用户密码
     */
    ...
    public Map<String,Object> updatePassword(User entity){
        ...
        try{
            resultMap = userService.updatePassword(entity);
        }catch(Exception e){
            e.printStackTrace();
            resultMap = ResponseData.returnResponseData(false,"操作失败",null);
        }
